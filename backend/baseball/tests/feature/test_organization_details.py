...
class TestOrganizationDetailsApi (APITestCase):
    ...
    def test_organization_by_id(self):
        ...
        """
        response = self.client.get(f'/organizations/{self.test_organization_id}/')
    ...
    def test_edit_organization(self):
        ...
        response = self.client.put(path=f'/organizations/{self.test_organization_id}/', data=updated_organization_field, format='json')
        self.assertEqual(status.HTTP_200_OK, response.status_code)
    ...
    def test_delete_organization(self):
        ...
        """
        delete_response = self.client.delete(path=f'/organizations/{self.test_organization_id}/')
        self.assertEqual(status.HTTP_204_NO_CONTENT, delete_response.status_code)

        get_response = self.client.get(path=f'/organizations/{self.test_organization_id}/')
